...
kRapidUploadThreshold = 256 * 1024
...
def enable_share(cookie, tokens, fid_list):
    ...
    req = net.urlopen(url, headers={
        "Cookie": cookie.header_output(),
        "Content-type": net.kContentFormUtf8,
        }, data=data.encode())
    if req:
        content = req.data
        return json.loads(content.decode())
...
def disable_share(cookie, tokens, shareid_list):
    ...
    """
    url = "".join([
        net.kPanUrl, "share/cancel?channel=chunlei&clienttype=0&web=1",
        "&bdstoken=", tokens["bdstoken"],
    ])
...
def share_transfer(cookie, tokens, shareid, uk, filelist, dest, upload_mode):
    ...
    """
    ondup = states.kUploadOnDup[upload_mode]
    url = "".join([
        net.kPanUrl,
...
#    ])
#    resp = net.urloption(url, headers={'Accept': net.kAcceptHtml})
#    if resp:
...
def upload(cookie, source_path, path, upload_mode):
    ...
    headers = {"Accept": net.kAcceptHtml, "Origin": net.kPanUrl}
...
def rapid_upload(cookie, tokens, source_path, path, upload_mode):
    ...
    assert content_length > kRapidUploadThreshold, "file size is not satisfied!"
    dir_name, file_name = os.path.split(path)
    content_md5 = hasher.md5(source_path)
    slice_md5 = hasher.md5(source_path, 0, kRapidUploadThreshold)
    url = "".join([
        net.kPcs,
