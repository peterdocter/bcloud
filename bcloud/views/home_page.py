...
class PathBox(Gtk.Box):
    ...
    def append_button(self, abspath, name):
        ...
        if abspath == "/":
            ...
            button.set_label(util.ellipse_text(name))

        if not util.kGtkGe312:
    ...
    def add_view_history(self, abs_path):
        logger.debug("[PathBox.add_view_history] %s" % abs_path)
...
#        util.async_call(pcs.list_dir, self.app.cookie, self.app.tokens,
#                         self.path, self.page_num, callback=self.on_load)
#        util.async_call(pcs.get_quota, self.app.cookie, self.app.tokens,
...
#                self.has_next = False
    ...
    def on_search_entry_activated(self, search_entry):
        ...
        self.loading_spin.start()
        self.loading_spin.show_all()
        #util.async_call(pcs.search, self.app.cookie, self.app.tokens, text,
