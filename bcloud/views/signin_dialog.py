...
class SigninDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, auto_signin):
        super().__init__(_("Sign in now"), parent, Gtk.DialogFlags.MODAL)
        ...
        for username in Profiles().profiles:
        ...
        self.password_entry.connect("activate", self.signin)
        ...
        self.signin_button.connect("clicked", self.signin)
        ...
        self.load_defualt_profile()

    def load_defualt_profile(self):
        if Profiles().default:
            self.use_profile(Profiles().default)
    ...
    def use_profile(self, username):
        ...
        for row in model: 
            if row[0] == username:
                ...
                break
        self.password_entry.set_text(keyring.read_password(username))
        if keyring.is_available:
            if Settings().remember_password:
                self.remember_check.set_active(True)
                self.signin_check.set_active(Settings().auto_signin)
            else:
                self.remember_check.set_active(False)
                self.signin_check.set_active(False)
        else:
    ...
    def on_remember_check_toggled(self, button):
        ...
        if Settings().username:
            Settings().remember_password = button.get_active()

    def on_signin_check_toggled(self, button):
        if Settings().username:
            Settings().auto_signin = button.get_active()
    ...
    def signin(self, *args):
        if (len(self.password_entry.get_text()) <= 1 or
            ...
            return
        self.infobar.hide()
        self.signin_button.set_label(_("Connecting to server..."))
        self.signin_button.set_sensitive(False)
