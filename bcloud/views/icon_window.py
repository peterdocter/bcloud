...
#from .folder_chooser_dialog import FolderChooserDialog
#from .folder_properties_dialog import FolderPropertyDialog
...
(kPixbufCol, kNameCol, kPathCol, kTooltipCol, kSizeCol, kHumanSizeCol,
 kIsDirCol, kMtimeCol, kHumanMtimeCol, kTypeCOl, kPcsFileCol) = tuple(range(11))
kTypeTorrent = "application/x-bittorrent"

kDragTargets = (
    # 用于拖拽下载
    (TargetType.kUriList, Gtk.TargetFlags.OTHER_APP, TargetInfo.kUriList),
    # 用于移动文件到子目录
    (TargetType.kPlainText, Gtk.TargetFlags.SAME_WIDGET, TargetInfo.kPlainText),
)
kDropTargets = (
    (TargetType.kPlainText, Gtk.TargetFlags.SAME_WIDGET, TargetInfo.kPlainText),
)

kDragTargetList = [Gtk.TargetEntry.new(*t) for t in kDragTargets]
kDropTargetList = [Gtk.TargetEntry.new(*t) for t in kDropTargets]
kDragAction = Gdk.DragAction.MOVE
...
class IconWindow(Gtk.ScrolledWindow):
    ...
    def init_ui(self):
        ...
        self.iconview.set_pixbuf_column(kPixbufCol)
        self.iconview.set_text_column(kNameCol)
        self.iconview.set_tooltip_column(kTooltipCol)
        self.iconview.set_item_width(84)
        self.iconview.set_selection_mode(Gtk.SelectionMode.MULTIPLE)

        self.iconview.enable_model_drag_source(Gdk.ModifierType.BUTTON1_MASK,
                                               kDragTargetList, kDragAction)
        self.iconview.connect("drag-data-get", self.on_drag_data_get)
        self.iconview.enable_model_drag_dest(kDragTargetList, kDragAction)
        ...
        self.liststore.set_sort_func(kNameCol, util.tree_model_natsort)
    ...
    def display_files(self, pcs_files):
        ...
        for pcs_file in pcs_files:
            ...
            tooltip = util.escape_tooltip(name)
        ...
        util.async_call(util.update_liststore_image, self.liststore,
                        tree_iters, kPixbufCol, pcs_files, cache_path,
                        self.kIconSize)

    def get_pcs_file(self, tree_path):
        """获取原始的pcs文件信息"""
        return json.loads(self.liststore[tree_path][kPcsFileCol])

    def on_scrolled(self, adj):
        if util.reach_scrolled_bottom(adj) and self.parent.has_next:
    ...
    def on_drag_data_received(self, widget, context, x, y, data, info, time):
        ...
        if tree_path is None:
            return
        target_path = self.liststore[tree_path][kPathCol]
        is_dir = self.liststore[tree_path][kIsDirCol]
        if not is_dir or info != TargetInfo.kPlainText:
            return
        ...
        for file_item in filelist:
            if file_item["path"] == target_path:
                notify(_("Error: Move folder to itself!"))
                return
        for file_item in filelist:
            file_item["dest"] = target_path
        #util.async_call(pcs.move, self.app.cookie, self.app.tokens, filelist,
        #                 callback=self.parent.reload)

    def on_iconview_item_activated(self, iconview, tree_path):
        path = self.liststore[tree_path][kPathCol]
        type_ = self.liststore[tree_path][kTypeCOl]
        if type_ == "folder":
    ...
    def popup_item_menu(self, event):
        ...
        if len(tree_paths) == 1:
            ...
            file_type = self.liststore[tree_path][kTypeCol]
            if file_type == "folder":
    ...
    def launch_app_with_app_info(self, app_info):
        def open_video_link(red_url, error=None):
            ...
            if error or not red_url:
                logger.error("IconWindow.launch_app_with_app_info: %s, %s" %
                             (red_url, error))
                return
            util.async_call(app_info.launch_uris, [red_url, ], None)
        ...
        def save_playlist(pls, error=None):
            ...
            if error or not pls or b"error_code" in pls:
                util.async_call(pcs.get_download_link, self.app.cookie,
                                self.app.tokens,
                                self.liststore[tree_paths[0]][kPathCol],
        ...
        if "video" in file_type or "media" in file_type:
            if self.app.profile["use-streaming"]:
                util.async_call(pcs.get_streaming_playlist, self.app.cookie,
    ...
    def on_open_dir_item_activated(self, menu_item):
        ...
        if tree_paths and len(tree_paths) == 1:
            self.parent.load(self.liststore[tree_paths[0]][kPathCol])
    ...
    def on_share_activated(self, menu_item):
        def on_share(info, error=None):
            if error or not info or info["errno"] != 0:
                logger.error("IconWindow.on_share_activated: %s, %s" %
                             (info, error))
                notify(_("Failed to share selected files"))
                return
        ...
        for tree_path in tree_paths:
            ...
            util.async_call(pcs.enable_share, self.app.cookie, self.app.tokens,
    ...
    def on_private_share_activated(self, menu_item):
        ...
        for tree_path in tree_paths:
            pcs_file = self.get_pcs_file(tree_path)
            fid_list.append(pcs_file["fs_id"])
            util.async_call(pcs.enable_private_share, self.app.cookie,
    ...
    def on_copyto_activated(self, menu_item):
        ...
        util.async_call(pcs.copy, self.app.cookie, self.app.tokens, filelist,
                         callback=self.parent.reload)
    ...
    def on_rename_activated(self, menu_item):
        ...
        path_list = []
        for tree_path in tree_paths:
            path_list.append(self.liststore[tree_path][kPathCol])
    ...
    def on_trash_activated(self, menu_item):
        ...
        for tree_path in tree_paths:
            path_list.append(self.liststore[tree_path][kPathCol])
        util.async_call(pcs.delete_files, self.app.cookie, self.app.tokens,
...
class TreeWindow(IconWindow):
    ...
    def init_ui(self):
        ...
        self.liststore.set_sort_func(NAME_COL, util.tree_model_natsort)

        size_cell = Gtk.CellRendererText()
        size_col = Gtk.TreeViewColumn(_("Size"), size_cell, text=kHumaSizeCol)
        ...
        mtime_col.set_sort_column_id(kMtimeCol)
